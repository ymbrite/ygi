...
Из физических единиц ещё может представлять интерес `q`: это относительно новая единица, и она поддерживается [не всеми браузерами](https://caniuse.com/#feat=mdn-css_types_length_q). `q` — это `0.25mm`. Как и другие физические единицы, больше имеет смысл для печати, но также можно попытаться использовать её для уменьшения размера кода: `q` — это примерно `0.945px`, то есть в некоторых случаях вполне можно использовать её вместо пикселей, получается один символ (`q`) вместо двух (`px`).
